...
- ✅ **H3 RESUELTO (2026-08-26)**: leads del bot tubandera → Supabase (`public.leads`) con `guardar_lead()`; bot ya no escribe sqlite como fuente. Puertos personas_db corregido (5433→5434), cast `str(chat_id)` (mata `text=bigint`), DB_DIR `03_Infrastructure`→`data/`, bot+guards añadidos a deploy.sh. Commit `22453004`+`ed67ee01`. Lección: (a) restart systemd deja proceso huérfano compitiendo por getUpdates → matar y verificar MainPID; (b) repo VPS tenía versión obsoleta de 78 líneas que abortaba `git pull` → `git stash` + `pull --ff-only`.
- ✅ **H4 RESUELTO (2026-08-26)**: guardas anti-injection robustecidas (`guards.py`). Refutador detectó 2/14 vectores de exfiltración de prompt que pasaban → añadidos verbos (exhibe/expón/describe/detalla/enseñame) + targets (soul/persona/directrices/estructura/detalles/código). Test: 11/11 ataques bloqueados, 6/6 legítimos OK (0 falsos positivos). Commit `d8f6a9b3`, verificado en prod. Lección: verificación adversarial en vivo > leer código; usar archivo .py temporal (no heredoc SSH) para test guards.
