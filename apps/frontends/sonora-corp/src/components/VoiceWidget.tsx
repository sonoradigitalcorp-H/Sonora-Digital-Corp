...
export default function VoiceWidget({ tenant = "abe" }: { tenant?: string }) {
        ...
        if (d.type === "chat_response" || d.type === "audio_response") {
          ...
          if (d.audio) {
            ...
            audio.play().catch(() => {
              ...
              setStatus("Audio disponible (reproduce automáticamente)");
            });
          }
          setStatus("Presiona el micrófono");
        }
        if (d.type === "connected") {
          setStatus("Listo. Presiona el micrófono");
        }
  ...
  const sendMessage = useCallback((text: string, fromVoice: boolean = false) => {
    if (!text.trim()) return;
    ...
    if (wsRef.current?.readyState === WebSocket.OPEN) {
      wsRef.current.send(JSON.stringify({
        ...
        audio: fromVoice,  // Siempre pedir audio en respuestas
      }));
    }
    setShowInput(false);
    setInputText("");
