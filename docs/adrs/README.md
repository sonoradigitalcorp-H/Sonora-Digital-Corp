# Architecture Decision Records

Each ADR documents a significant architecture decision, including context, options considered, and the decision.

**Format**: `ADR-YYYYMMDD-NNN-<title-slug>.md`
**Template**: `.specify/templates/adr-template.md`
**Lifecycle**: proposed → accepted → deprecated → superseded

## Index

| ADR | Title | Status | Date |
|-----|-------|--------|------|
| ADR-20260703-A | System Architecture Foundation | accepted | 2026-07-03 |
| ADR-20260704-ABE-001 | ABE Music Client Architecture | accepted | 2026-07-04 |
| ADR-20260718-CLONE-SERVICE | Clone Service Design | accepted | 2026-07-18 |
| ADR-20260718-ONBOARDING | Onboarding Flow Architecture | accepted | 2026-07-18 |
| ADR-20260719-PRODUCTOS-NUEVOS | Productos Nuevos SDK | accepted | 2026-07-19 |
| ADR-20260719-SDK-PYTHON | Python SDK Design | accepted | 2026-07-19 |
| ADR-20260719-SKILL-STANDARD | Skill Standardization | accepted | 2026-07-19 |
| ADR-20260719-UNIFICACION-ECOSISTEMAS | Ecosystem Unification | accepted | 2026-07-19 |
| ADR-20260719-WHATSAPP-OS-FASE1 | WhatsApp OS Phase 1 | accepted | 2026-07-19 |
| ADR-20260721-SDD-FRAMEWORK | SDD Framework Implementation | accepted | 2026-07-21 |
| ADR-20260722-001 | Multi-Tenant Core/Tenant Architecture | proposed | 2026-07-22 |
| ADR-20260802-AZROTECH-MVP-RAG-MEMORIA | Aztrotech MVP RAG-First + Memoria | accepted | 2026-08-02 |
| ADR-20260802-AZROTECH-VOZ-LOCAL | Aztrotech Voz Local (edge-tts + faster-whisper) | accepted | 2026-08-02 |
| ADR-20260802-AZROTECH-WHATSAPP-SANDBOX | Aztrotech WhatsApp Sandbox (solo 6623538272) | accepted | 2026-08-02 |
| ADR-20260802-AZROTECH-GITHUB-CI | Aztrotech GitHub CI + Sync Memorias | accepted | 2026-08-02 |
| ADR-20260802-SDC-SYSTEM-SESSION | Sesión completa: auditoría, fixes, protección, automatización, JARVIS 3D | accepted | 2026-08-02 |
