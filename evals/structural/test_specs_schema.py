...
def test_specs_index_exists():
    path = os.path.join(REPO, "specs", "index.yaml")
...
def test_specs_schema_exists():
    path = os.path.join(REPO, "specs", "schema", "spec-v1.yaml")
    assert os.path.exists(path)


def test_all_capabilities_have_specs():
    index_path = os.path.join(REPO, "specs", "index.yaml")
    with open(index_path) as f:
        index = yaml.safe_load(f)

    for cap in index["capabilities"]:
        spec_path = os.path.join(REPO, cap["path"], "spec.md")
