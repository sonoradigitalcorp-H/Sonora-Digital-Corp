...
const FALLBACK = { name: "", total_streams: 0, total_revenue: 0, telegram_handle: "" };
        ...
        <div className="glass rounded-3xl p-8 md:p-12 mb-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            <div className="glass rounded-xl p-5 text-center">
              <div className="text-3xl font-bold gradient-text">${Number(artist.total_revenue || 0).toLocaleString()}</div>
              <div className="text-xs text-gray-500 mt-1">Revenue</div>
            </div>
            <div className="glass rounded-xl p-5 text-center">
              <div className="text-3xl font-bold text-[#FF6B35]">{(Number(artist.total_streams || 0) / 1000000).toFixed(1)}M</div>
              <div className="text-xs text-gray-500 mt-1">Streams</div>
            </div>
            <div className="glass rounded-xl p-5 text-center">
              <div className="text-3xl font-bold text-[#00ccff]">{((Number(artist.total_revenue || 0) / Math.max(Number(artist.total_streams || 0), 1)) * 1000).toFixed(4)}</div>
          ...
          <div className="glass rounded-2xl p-6">
            ...
            <div className="grid md:grid-cols-2 gap-4">
              <div className="glass rounded-xl p-5">
                <div className="text-lg font-bold">{Number(artist.total_streams || 0).toLocaleString()}</div>
                <div className="text-xs text-gray-500">Streams totales</div>
              </div>
              <div className="glass rounded-xl p-5">
                <div className="text-lg font-bold">${Number(artist.total_revenue || 0).toLocaleString()}</div>
        ...
        {tab === "bot" && (
          <div className="space-y-4">
            <div className="glass rounded-2xl p-6">
              ...
              {artist.telegram_handle && (
                <a href={`https://t.me/${artist.telegram_handle}`} target="_blank"
                  className="mt-4 flex items-center gap-2 text-sm text-[#26a5e4] hover:underline">
                  <MessageCircle className="w-4 h-4" /> @{artist.telegram_handle} en Telegram
                </a>
              )}
            </div>
          </div>
