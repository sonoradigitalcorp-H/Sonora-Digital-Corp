...
const OLLAMA_SYSTEM = `Eres un asistente de IA de Sonora Digital Corp.
Ayudas a los clientes con lo que necesiten: información, automatización, código, estrategia de negocio y más.
...
// ── OpenClaw Gateway ────────────────────────────────────────────────────────────
const OPENCLAW_BASE = process.env.OPENCLAW_BASE || 'http://localhost:18789';
const OPENCLAW_ENABLED = process.env.OPENCLAW_ENABLED !== 'false'; // true by default

const { execSync } = require('child_process');

async function askOpenClaw(message, tenantId, userId) {
  const startTime = Date.now();
  try {
    const output = execSync(
      `openclaw agent --agent main --message ${JSON.stringify(message)} --json`,
      { timeout: 25000, encoding: 'utf-8', shell: true, env: { ...process.env, OLLAMA_API_KEY: 'sdc-key' } }
    );
    ...
    if (reply) {
      traceLangFuse('telegram.askOpenClaw', { message, tenantId },
        { reply_length: reply.length },
        { duration_ms: Date.now() - startTime, cost_usd: 0, model: 'openclaw', agent: 'telegram.openclaw', tenant: tenantId }
      );
      return reply;
    }
  } catch (e) {
    const ec = e.code || (e.stderr || '').slice(0, 100);
    if (ec !== 'ECONNREFUSED' && ec !== 'ECONNRESET' && !String(ec).includes('gateway connect failed')) {
      log.warn(`[OpenClaw] error: ${e.message}`);
    }
  }
  return null; // fallback a Ollama
}

// ── Multi-tenant bots (declaración temprana para skillTenants) ────────────────
const BOT_TOKENS = [
...
// Skills brain-ask y ayuda-menu aplican a TODOS los tenants activos.
...
function skillTenants(skill) {
  ...
  if (name === 'brain-ask' || name === 'ayuda-menu') return BOT_TOKENS.filter(b => b.token).map(b => b.id);
  return ['default'];
}
...
async function invokeSkill(skill, message, tenantId) {
  ...
  if (skill.method === 'STATIC') {
    return skill.response_text || 'Información no disponible.';
  }
  // OPENCLAW — reenvía a OpenClaw Gateway con contexto del skill
  if (skill.method === 'OPENCLAW' && OPENCLAW_ENABLED) {
    const question = (skill.question_template || '{{message}}')
      .replace('{{message}}', message);
    const reply = await askOpenClaw(question, tenantId, 'system');
    return reply || 'No pude procesar tu consulta.';
  }
...
function createTelegrafInstance(tenantId, token) {
  ...
  const greetings = {
    ...
    'sonora-digital': '🏢 *Sonora Digital Corp*\n\nSoy el asistente oficial de SDC. Puedo ayudarte con información sobre nuestros servicios, automatización, tecnología y más.\n\n_¿En qué te ayudo hoy?_',
    'aztrotech': '🚀 *AztroTech*\n\nSoy el asistente de AztroTech. Estoy aquí para ayudarte con ventas, reservaciones y lo que necesites.\n\n_¿En qué te ayudo hoy?_',
    'abe-personal': '🎵 *ABE Personal*\n\nSoy el asistente personal de Abraham. Estoy aquí para ayudarte.\n\n_¿En qué te ayudo hoy?_',
    'mystika': '✨ *Mystika*\n\nSoy el asistente de Mystika. ¿En qué puedo ayudarte hoy?_',
  };
  bot.start(async ctx => {
    try {
      const greeting = greetings[tenantId] ||
        `✨ *¡Hola!*\n\nSoy un asistente de IA de Sonora Digital Corp, disponible las 24 horas.\n\n` +
        `Pregúntame lo que necesites — información, automatización, tecnología, servicios o cualquier duda.\n\n` +
    ...
    try {
      await ctx.reply('¿Qué necesitas hoy?', {
        ...
        reply_markup: { inline_keyboard: [
          [{ text: '💬 Hacer una pregunta',       callback_data: 'action_help' }],
          [{ text: 'ℹ️ Información del servicio', callback_data: 'action_info' }],
          [{ text: '📞 Hablar con un humano',     callback_data: 'action_human' }],
        ]}
      });
    } catch (err) {
  ...
  bot.on('text', async ctx => {
    ...
    try {
      ...
      if (skill) {
        ...
        text = await askOpenClaw(msg, tenantId, userId);
        if (!text) text = await askOllama(msg);
      } else {
  ...
  bot.on('callback_query', async ctx => {
    try {
      ...
      const replies = {
        action_help:  '💬 Claro, pregúntame lo que necesites. Estoy aquí para ayudarte.',
        action_info:  'ℹ️ Soy un asistente de IA de Sonora Digital Corp. Puedo ayudarte con información, automatización, tecnología y más. ¿Sobre qué tema te gustaría saber más?',
        action_human: '📞 Tomaré nota de tu solicitud. Un humano se pondrá en contacto contigo pronto.',
      };
...
// GET /list — lista todos los bots con su estado
app.get('/list', (req, res) => {
  const bots = BOT_TOKENS.map(b => ({
    id: b.id,
    active: botInstances.has(b.id),
    hasToken: !!b.token,
  }));
  res.json({ bots, activeCount: botInstances.size, total: BOT_TOKENS.length });
});

// POST /send — envía un mensaje a través de un bot específico
app.post('/send', async (req, res) => {
  const { tenantId, chatId, message, parseMode } = req.body;
  if (!tenantId || !chatId || !message) {
    return res.status(400).json({ error: 'Faltan campos: tenantId, chatId, message' });
  }
  const bot = botInstances.get(tenantId);
  if (!bot) return res.status(404).json({ error: `Bot ${tenantId} no está activo` });
  try {
    await bot.telegram.sendMessage(chatId, message, { parse_mode: parseMode || 'Markdown' });
    res.json({ success: true, tenantId, chatId });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});

// POST /broadcast — envía un mensaje a través de todos los bots activos
app.post('/broadcast', async (req, res) => {
  const { chatId, message, parseMode } = req.body;
  if (!chatId || !message) {
    return res.status(400).json({ error: 'Faltan campos: chatId, message' });
  }
  const results = [];
  for (const [tid, bot] of botInstances) {
    try {
      await bot.telegram.sendMessage(chatId, message, { parse_mode: parseMode || 'Markdown' });
...
// GET /openclaw/config — devuelve la configuración para que OpenClaw se conecte
app.get('/openclaw/config', (req, res) => {
  const activeBots = BOT_TOKENS.filter(b => botInstances.has(b.id)).map(b => ({
    id: b.id, active: true
  }));
  res.json({
