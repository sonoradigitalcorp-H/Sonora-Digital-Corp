...
const tools = {
  files_list: { description: 'Lista archivos del proyecto', inputSchema: { type: 'object', properties: { path: { type: 'string' } } }, handler: async (args) => await api('GET', `/api/files?path=${encodeURIComponent(args.path || '.')}`) },
  files_git_status: { description: 'Estado de git del proyecto', inputSchema: { type: 'object', properties: {} }, handler: async () => await api('GET', '/api/files/git') },
};
