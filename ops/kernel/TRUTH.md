...
| `~/SonoraDigitalCorp-Yami/` | `🚫 OBSOLETO — eliminado, contenido migrado a root` |
| `$YAMI_HOME` | `🚫 OBSOLETO — eliminado, contenido migrado a root` |
| `archive-sonora-digital-corp-git/` | `🚫 OBSOLETO — movido a backups/repos/` |
| `/home/ubuntu/sdc/` | `✅ ~/sdc → /home/ubuntu/sonora-digital-corp/` |
...
| Enterprise OS | root (`skills/`, `harnesses/`, `initiatives/`, `metrics/`) |
| Kernel / Constitution | `kernel/` |
| OMEGA PROMPT | `kernel/OMEGA-PROMPT.md` |
| RULES | `kernel/10-RULES.md` |
| Soul | `kernel/SOUL.md` |
| Truth | `kernel/TRUTH.md` |
| CONTRATO | `kernel/CONTRATO.md` |
| Checksums | `kernel/CHECKSUMS.sha256` |
| State (logs, engram, events, quality) | `state/` |
| SDC core engine | `apps/core/` |
| Events system | `state/events/` |
| Evolution engine | `apps/evolution/` |
| Tenants config | `config/tenants/` |
| Web UI | `apps/webui/` (FastAPI, port 5174) |
| Skills (10 canonical) | `skills/` |
| Specs (active) | `process/specs/` + `process/active/` |
| Quality violations | `state/quality/violations.jsonl` |
| Docker compose | `infra/docker-compose.yml` |
| Tests | `tests/` |
| Scripts | `scripts/` |
| SDD CLI | `scripts/tools/sdd/` |
| Config files | `config/` |
| Fleet config (SSOT) | `infra/fleet.yml` |
| Backups | `backups/` |
| Portal (Grimoire 3D) | `portal/` |
| Evaluations | `tests/evals/` |
...
## Agent Registry (`config/agents/registry.yaml`)
...
- **10 Agent Harnesses**: all defined in `harnesses/`
- **10 Canonical Skills**: all defined in `skills/`
