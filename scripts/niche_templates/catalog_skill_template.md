# niche-{{SLUG}}-catalog — {{NICHE_NAME}} Catalog Skill

**Inherits**: OMEGA PROMPT v10.0 + SOUL.md
**Template Version**: 1.0.0
**Audit ID**: SKILL-NICHE-{{NICHE_ID}}-CAT-001
...
```
Business Objective: Allow {{NICHE_NAME}} clients to browse and request niche services via WhatsApp using the catalog.
```
...
```gherkin
Given a client sends a WhatsApp message to the {{NICHE_NAME}} niche
And the text contains catalog triggers: {{TRIGGERS}}
...
```gherkin
Then the {{NICHE_NAME}} catalog is formatted as a WhatsApp-friendly message
...
```
Events:
- niche:{{NICHE_ID}}:catalog:requested: client asks for catalog
- niche:{{NICHE_ID}}:service:selected: client picks a service
- niche:{{NICHE_ID}}:catalog:sent: catalog delivered
- niche:{{NICHE_ID}}:lead:qualified: client intent identified
```
...
```
Dependencies:
- state/niches/{{SLUG}}/catalog.json: niche service catalog (data)
- state/niches/{{SLUG}}/pricing.json: niche pricing (data)
...
```
Recovery Procedure:
1. If catalog missing → load default {{NICHE_ID}} backup from templates
2. If wacli down → queue response in SQLite retry table
3. If unknown service → ask clarifying question
4. Log all requests to state/logs/niche-{{SLUG}}-catalog.log
```
...
```
Business Value: Turns WhatsApp into a 24/7 {{NICHE_NAME}} sales channel. Estimated 30% increase in qualified leads per niche.
```
...
- Events: niche:{{NICHE_ID}}:catalog:requested, niche:{{NICHE_ID}}:service:selected, niche:{{NICHE_ID}}:catalog:sent, niche:{{NICHE_ID}}:lead:qualified
- Logs: state/logs/niche-{{SLUG}}-catalog.log
```
