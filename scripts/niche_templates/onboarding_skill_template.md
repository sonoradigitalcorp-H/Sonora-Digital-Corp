# niche-{{SLUG}}-onboarding — {{NICHE_NAME}} Onboarding Skill

**Inherits**: OMEGA PROMPT v10.0 + SOUL.md
**Template Version**: 1.0.0
**Audit ID**: SKILL-NICHE-{{NICHE_ID}}-ONB-001
...
```
Business Objective: Convert incoming WhatsApp leads into onboarded {{NICHE_NAME}} clients without human intervention.
```
...
And the webhook emits "niche:{{NICHE_ID}}:message:received"
And the message contains keywords: {{TRIGGERS}}
When the {{NICHE_ID}} onboarding agent processes the message
```
...
```gherkin
Then the client receives a welcome message with {{NICHE_NAME}} services
And the niche catalog is shared via WhatsApp
And a lead record is created in the CRM
And the client can request any of: {{SERVICES_COMMA}}
```
...
```
Events:
- niche:{{NICHE_ID}}:message:received: incoming message detected
- niche:{{NICHE_ID}}:onboarding:started: new client begins onboarding
- niche:{{NICHE_ID}}:onboarding:completed: client finishes onboarding
- niche:{{NICHE_ID}}:catalog:sent: catalog delivered via WhatsApp
```
...
- state/niches/{{SLUG}}/catalog.json: service catalog (data)
- state/niches/{{SLUG}}/pricing.json: pricing data (data)
...
```
Recovery Procedure:
1. If catalog missing → load default {{NICHE_ID}} backup
2. If wacli down → queue in SQLite retry table
3. If unknown request → ask clarifying question
4. Log all failures to state/logs/niche-{{SLUG}}-onboarding.log
```
...
```
Business Value: Automates 100% of {{NICHE_NAME}} inbound onboarding via WhatsApp. Estimated 10h/week saved per niche.
```
...
```
Audit Trail:
- ADR: ADR-20260719-NICHE-{{NICHE_ID}}
- Events: niche:{{NICHE_ID}}:message:received, niche:{{NICHE_ID}}:onboarding:started, niche:{{NICHE_ID}}:onboarding:completed, niche:{{NICHE_ID}}:catalog:sent
- Logs: state/logs/niche-{{SLUG}}-onboarding.log
```
