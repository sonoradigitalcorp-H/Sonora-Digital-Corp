...
alias clone-test="cd $REPO && PYTHONPATH=. python3 -m pytest tests/test_clone_*.py -q"
alias clone-ls="ls -la $REPO/gherkin/clone-*.feature $REPO/tests/test_clone_*.py $REPO/mcp/servers/*clone*.py $REPO/mcp/servers/generate_mcp.py $REPO/mcp/servers/credit_mcp.py $REPO/scripts/clone_pipeline.py"
alias clone-spec="cd $REPO && (bat process/active/SPEC-20260718-CLONE-SERVICE.md 2>/dev/null || cat process/active/SPEC-20260718-CLONE-SERVICE.md)"
...
alias clone-flow="bash $REPO/scripts/test_clone_flow.sh"
clone-status() {
  cd $REPO
...
alias brain-sync="bash $REPO/scripts/sync-brain-vault.sh"
alias brain-open="nohup $HOME/Applications/Obsidian.AppImage --no-sandbox $HOME/Documents/sdc-brain-vault > /dev/null 2>&1 & disown"
alias brain-status="echo '🧠 CEREBRO DIGITAL — Luis Daniel Guerrero Enciso'
echo '  Vault: ~/Documents/sdc-brain-vault'
ls $HOME/Documents/sdc-brain-vault/Observations/ 2>/dev/null | wc -l | xargs -I{} echo '  Observaciones: {}'
ls $HOME/Documents/sdc-brain-vault/People/ 2>/dev/null | wc -l | xargs -I{} echo '  Personas: {}'
ls $HOME/Documents/sdc-brain-vault/Decisions/ 2>/dev/null | wc -l | xargs -I{} echo '  Decisiones: {}'
ls $HOME/Documents/sdc-brain-vault/Projects/ 2>/dev/null | wc -l | xargs -I{} echo '  Proyectos: {}'
