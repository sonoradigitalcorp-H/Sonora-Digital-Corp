# niche-barberking-studio-catalog — BarberKing Studio Catalog Skill

**Inherits**: OMEGA PROMPT v10.0 + SOUL.md
**Template Version**: 1.0.0
**Audit ID**: SKILL-NICHE-barbero-CAT-001
...
```
Business Objective: Allow BarberKing Studio clients to browse and request niche services via WhatsApp using the catalog.
```
...
```gherkin
Given a client sends a WhatsApp message to the BarberKing Studio niche
And the text contains catalog triggers: corte, barbería, peluquería
...
```gherkin
Then the BarberKing Studio catalog is formatted as a WhatsApp-friendly message
...
```
Events:
- niche:barbero:catalog:requested: client asks for catalog
- niche:barbero:service:selected: client picks a service
- niche:barbero:catalog:sent: catalog delivered
- niche:barbero:lead:qualified: client intent identified
```
...
```
Dependencies:
- state/niches/barberking-studio/catalog.json: niche service catalog (data)
- state/niches/barberking-studio/pricing.json: niche pricing (data)
...
```
Recovery Procedure:
1. If catalog missing → load default barbero backup from templates
2. If wacli down → queue response in SQLite retry table
3. If unknown service → ask clarifying question
4. Log all requests to state/logs/niche-barberking-studio-catalog.log
```
...
```
Business Value: Turns WhatsApp into a 24/7 BarberKing Studio sales channel. Estimated 30% increase in qualified leads per niche.
```
...
- Events: niche:barbero:catalog:requested, niche:barbero:service:selected, niche:barbero:catalog:sent, niche:barbero:lead:qualified
- Logs: state/logs/niche-barberking-studio-catalog.log
```
