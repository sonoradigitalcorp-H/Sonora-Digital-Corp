# niche-barberking-studio-onboarding — BarberKing Studio Onboarding Skill

**Inherits**: OMEGA PROMPT v10.0 + SOUL.md
**Template Version**: 1.0.0
**Audit ID**: SKILL-NICHE-barbero-ONB-001
...
```
Business Objective: Convert incoming WhatsApp leads into onboarded BarberKing Studio clients without human intervention.
```
...
And the webhook emits "niche:barbero:message:received"
And the message contains keywords: corte, barbería, peluquería
When the barbero onboarding agent processes the message
```
...
```gherkin
Then the client receives a welcome message with BarberKing Studio services
And the niche catalog is shared via WhatsApp
And a lead record is created in the CRM
And the client can request any of: Corte de cabello, Fotos de perfil, Video promocional
```
...
```
Events:
- niche:barbero:message:received: incoming message detected
- niche:barbero:onboarding:started: new client begins onboarding
- niche:barbero:onboarding:completed: client finishes onboarding
- niche:barbero:catalog:sent: catalog delivered via WhatsApp
```
...
- state/niches/barberking-studio/catalog.json: service catalog (data)
- state/niches/barberking-studio/pricing.json: pricing data (data)
...
```
Recovery Procedure:
1. If catalog missing → load default barbero backup
2. If wacli down → queue in SQLite retry table
3. If unknown request → ask clarifying question
4. Log all failures to state/logs/niche-barberking-studio-onboarding.log
```
...
```
Business Value: Automates 100% of BarberKing Studio inbound onboarding via WhatsApp. Estimated 10h/week saved per niche.
```
...
```
Audit Trail:
- ADR: ADR-20260719-NICHE-barbero
- Events: niche:barbero:message:received, niche:barbero:onboarding:started, niche:barbero:onboarding:completed, niche:barbero:catalog:sent
- Logs: state/logs/niche-barberking-studio-onboarding.log
```
