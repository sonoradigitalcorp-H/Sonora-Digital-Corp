# niche-bufete-juridico-catalog — Bufete Jurídico Catalog Skill

**Inherits**: OMEGA PROMPT v10.0 + SOUL.md
**Template Version**: 1.0.0
**Audit ID**: SKILL-NICHE-abogado-CAT-001
...
```
Business Objective: Allow Bufete Jurídico clients to browse and request niche services via WhatsApp using the catalog.
```
...
```gherkin
Given a client sends a WhatsApp message to the Bufete Jurídico niche
And the text contains catalog triggers: abogado, consulta, legal
...
```gherkin
Then the Bufete Jurídico catalog is formatted as a WhatsApp-friendly message
...
```
Events:
- niche:abogado:catalog:requested: client asks for catalog
- niche:abogado:service:selected: client picks a service
- niche:abogado:catalog:sent: catalog delivered
- niche:abogado:lead:qualified: client intent identified
```
...
```
Dependencies:
- state/niches/bufete-juridico/catalog.json: niche service catalog (data)
- state/niches/bufete-juridico/pricing.json: niche pricing (data)
...
```
Recovery Procedure:
1. If catalog missing → load default abogado backup from templates
2. If wacli down → queue response in SQLite retry table
3. If unknown service → ask clarifying question
4. Log all requests to state/logs/niche-bufete-juridico-catalog.log
```
...
```
Business Value: Turns WhatsApp into a 24/7 Bufete Jurídico sales channel. Estimated 30% increase in qualified leads per niche.
```
...
- Events: niche:abogado:catalog:requested, niche:abogado:service:selected, niche:abogado:catalog:sent, niche:abogado:lead:qualified
- Logs: state/logs/niche-bufete-juridico-catalog.log
```
