# niche-bufete-juridico-onboarding — Bufete Jurídico Onboarding Skill

**Inherits**: OMEGA PROMPT v10.0 + SOUL.md
**Template Version**: 1.0.0
**Audit ID**: SKILL-NICHE-abogado-ONB-001
...
```
Business Objective: Convert incoming WhatsApp leads into onboarded Bufete Jurídico clients without human intervention.
```
...
And the webhook emits "niche:abogado:message:received"
And the message contains keywords: abogado, consulta, legal
When the abogado onboarding agent processes the message
```
...
```gherkin
Then the client receives a welcome message with Bufete Jurídico services
And the niche catalog is shared via WhatsApp
And a lead record is created in the CRM
And the client can request any of: Consulta legal, Redacción de contratos, Video asesoría
```
...
```
Events:
- niche:abogado:message:received: incoming message detected
- niche:abogado:onboarding:started: new client begins onboarding
- niche:abogado:onboarding:completed: client finishes onboarding
- niche:abogado:catalog:sent: catalog delivered via WhatsApp
```
...
- state/niches/bufete-juridico/catalog.json: service catalog (data)
- state/niches/bufete-juridico/pricing.json: pricing data (data)
...
```
Recovery Procedure:
1. If catalog missing → load default abogado backup
2. If wacli down → queue in SQLite retry table
3. If unknown request → ask clarifying question
4. Log all failures to state/logs/niche-bufete-juridico-onboarding.log
```
...
```
Business Value: Automates 100% of Bufete Jurídico inbound onboarding via WhatsApp. Estimated 10h/week saved per niche.
```
...
```
Audit Trail:
- ADR: ADR-20260719-NICHE-abogado
- Events: niche:abogado:message:received, niche:abogado:onboarding:started, niche:abogado:onboarding:completed, niche:abogado:catalog:sent
- Logs: state/logs/niche-bufete-juridico-onboarding.log
```
