# niche-mystik-music-onboarding — Mystik Music Onboarding Skill

**Inherits**: OMEGA PROMPT v10.0 + SOUL.md
**Template Version**: 1.0.0
**Audit ID**: SKILL-NICHE-musico-ONB-001
...
```
Business Objective: Convert incoming WhatsApp leads into onboarded Mystik Music clients without human intervention.
```
...
And the webhook emits "niche:musico:message:received"
And the message contains keywords: música, artista, cantante
When the musico onboarding agent processes the message
```
...
```gherkin
Then the client receives a welcome message with Mystik Music services
And the niche catalog is shared via WhatsApp
And a lead record is created in the CRM
And the client can request any of: Clon de voz, Fotos profesionales, Talking head
```
...
```
Events:
- niche:musico:message:received: incoming message detected
- niche:musico:onboarding:started: new client begins onboarding
- niche:musico:onboarding:completed: client finishes onboarding
- niche:musico:catalog:sent: catalog delivered via WhatsApp
```
...
- state/niches/mystik-music/catalog.json: service catalog (data)
- state/niches/mystik-music/pricing.json: pricing data (data)
...
```
Recovery Procedure:
1. If catalog missing → load default musico backup
2. If wacli down → queue in SQLite retry table
3. If unknown request → ask clarifying question
4. Log all failures to state/logs/niche-mystik-music-onboarding.log
```
...
```
Business Value: Automates 100% of Mystik Music inbound onboarding via WhatsApp. Estimated 10h/week saved per niche.
```
...
- Events: niche:musico:message:received, niche:musico:onboarding:started, niche:musico:onboarding:completed, niche:musico:catalog:sent
- Logs: state/logs/niche-mystik-music-onboarding.log
```
