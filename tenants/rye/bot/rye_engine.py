...
def answer(query: str, use_llm: bool = True) -> dict:
    ...
    # LLM
    from skills.mcp.servers.sdc_mcp_stdio import llm_chat
    messages = [
        {"role": "system", "content": build_system_prompt()},
        {"role": "user", "content": f"CONSULTA: {query}\n\nCONTEXTO:\n{context}\n\n{rag_ctx}".strip()},
    ]
    import asyncio
    out = asyncio.run(llm_chat(messages))
    data = json.loads(out)
...
def save_memory(query: str, response: str) -> None:
    from skills.mcp.servers.sdc_mcp_stdio import engram_save
    key = f"qa_{abs(hash(query)) % 1000000}"
    engram_save(TENANT, key, f"Q: {query} | R: {response[:300]}", tags="rye,qa")
